...
$(document).ready(function() {
   ...
   $('.open_edit_form').click(function(e) {
        ...
        $.ajax({
            ...
            $('#supplierForm input[name="supplier_address"]').val(obj.sonha);
            $.each(obj.tinh, function(index, value) {
                $('#supplier-city').append(
                    $('<option></option>').val(value.idTinh).text(value.tenTinh)
                );
            });

            $.each(obj.quan, function(index, value) {
                $('#supplier-district').append(
                    $('<option></option>').val(value.idQuan).text(value.tenQuan)
                );
            });

            $.each(obj.xa, function(index, value) {
                $('#supplier-ward').append(
                    $('<option></option>').val(value.idXa).text(value.tenXa)
                );
            });

            $('#supplier-city').val(parseInt(obj.idTinh));
            $('#supplier-district').val(obj.idQuan);
            $('#supplier-ward').val(obj.idXa);
            if (parseInt(obj.supplier.trangthai)) {
                $('#status').prop('checked', true);
                $('#switch-label').text('Đang hoạt động');
            } else {
                $('#status').prop('checked', false);
